...
async function findScryptFiles(dir: string): Promise<string[]> {
  const files = await readdir(dir)

  return files.filter((file) => file.split(".").pop() === "scrypt")
}
...
async function mdToHtml(filePath: string) {
  ...
  const scryptFileNames = await findScryptFiles(dir)

  const codes = {}
  for (const scryptFileName of scryptFileNames) {
    const source = (await readFile(path.join(dir, scryptFileName))).toString()
    // @ts-ignore
    codes[removeExt(scryptFileName)] = source
  }

  // render scrytp inside markdown
